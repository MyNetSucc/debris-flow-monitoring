...
def sync_status() -> bool:
    ...
    try:
        # The detector swaps this file in with an atomic rename; on Windows that can
        # briefly collide with this read (PermissionError), so retry before failing.
        data = None
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    print("[SYNC] status file locked by detector, skipping this round")
                    return False
                time.sleep(0.05)
        if data is None:
            return False
